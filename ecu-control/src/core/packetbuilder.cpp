...
void PacketBuilder::appendFloat(QByteArray &packet, float value)
{
    QDataStream stream(&packet, QIODevice::Append);
    stream.setByteOrder(QDataStream::LittleEndian);
    stream.setFloatingPointPrecision(QDataStream::SinglePrecision); // varsay─▒lan DoublePrecision, float'─▒ bile 8 byte yazar
    stream << value;
}
...
QByteArray PacketBuilder::buildPacket(const QVariantList &orderedValues)
{
    ...
    for (int i = 0; i < orderedValues.size(); ++i) {
        ...
        double stepSize = m_stepSizes.value(i, 1.0);  // tan─▒ml─▒ de─şilse varsay─▒lan: int davran─▒┼ş─▒

        if (stepSize < 1.0) {
            // Ondal─▒kl─▒ ad─▒m -> her zaman float (4 byte) olarak kodla.
            ...
            if (m_enumOptions.contains(i)) {
                ...
                if (idx >= 0) {
                    ...
                    qWarning() << "Bilinmeyen enum de─şeri:" << val.toString() << "index:" << i;
                }
            }
    ...
    // Checksum art─▒k 2 byte: paketteki t├╝m byte'lar─▒n 16-bit XOR'u.
    // Header (0xAA, 0x55) dahil, checksum alan─▒ hari├ğ t├╝m byte'lar ├╝zerinden hesaplan─▒r.
    quint16 checksum = 0;
    for (int i = 0; i < packet.size(); i += 2) {
        ...
        if (i + 1 < packet.size()) {
            ...
            // tek say─▒da byte kald─▒ysa (bu paket d├╝zeninde olmaz ama savunma ama├ğl─▒)
