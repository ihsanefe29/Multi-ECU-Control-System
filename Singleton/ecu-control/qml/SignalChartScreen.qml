...
//   - Dispatcher.signalLoader: loads a complete file at once, static plot
//   - Dispatcher.liveSource: mock "live" data arriving sample-by-sample,
...
Item {
    ...
    Text {
        ...
        anchors.right: parent.right
        anchors.margins: 16
        text: Dispatcher.signalLoader.lastError
        visible: Dispatcher.signalLoader.lastError.length > 0
        color: "#e6483a"
        font.pixelSize: 12
    ...
    Text {
        ...
        visible: !Dispatcher.signalLoader.loaded && !root.liveMode
        color: "#9aa4ad"
        font.pixelSize: 13
    }
    ...
    ColumnLayout {
        ...
        Text { visible: !root.liveMode; text: qsTr("Dosya: %1").arg(Dispatcher.signalLoader.fileName); color: "#9aa4ad"; font.pixelSize: 12; wrapMode: Text.WordWrap; Layout.fillWidth: true }
        Text { visible: !root.liveMode; text: qsTr("Örnek Sayısı: %1").arg(Dispatcher.signalLoader.sampleCount); color: "#9aa4ad"; font.pixelSize: 12 }
        Text { visible: !root.liveMode; text: qsTr("Örnekleme Hızı: %1 Hz").arg(Dispatcher.signalLoader.sampleRateHz.toFixed(2)); color: "#9aa4ad"; font.pixelSize: 12 }
        Text { visible: !root.liveMode; text: qsTr("Süre: %1 s").arg(Dispatcher.signalLoader.durationSeconds.toFixed(2)); color: "#9aa4ad"; font.pixelSize: 12 }

        // Live mode parameters - these are the signal's actual
        // generating parameters, unlike the file case where they're
        // just descriptive.
        Text { visible: root.liveMode; text: qsTr("Frekans: %1 Hz").arg(Dispatcher.liveSource.frequencyHz.toFixed(2)); color: "#9aa4ad"; font.pixelSize: 12 }
        Text { visible: root.liveMode; text: qsTr("Genlik: %1").arg(Dispatcher.liveSource.amplitude.toFixed(2)); color: "#9aa4ad"; font.pixelSize: 12 }
        Text { visible: root.liveMode; text: qsTr("Örnekleme Hızı: %1 Hz").arg(Dispatcher.liveSource.sampleRateHz.toFixed(2)); color: "#9aa4ad"; font.pixelSize: 12 }
    ...
    RowLayout {
        ...
        Text { text: qsTr("Min: %1").arg((root.liveMode ? root.liveMinValue : Dispatcher.signalLoader.minValue).toFixed(3)); color: "#e8ebee"; font.pixelSize: 13 }
        Text { text: qsTr("Maks: %1").arg((root.liveMode ? root.liveMaxValue : Dispatcher.signalLoader.maxValue).toFixed(3)); color: "#e8ebee"; font.pixelSize: 13 }
        Text { text: qsTr("Ortalama: %1").arg((root.liveMode ? root.liveMeanValue : Dispatcher.signalLoader.meanValue).toFixed(3)); color: "#e8ebee"; font.pixelSize: 13 }
        Text { text: qsTr("RMS: %1").arg((root.liveMode ? root.liveRmsValue : Dispatcher.signalLoader.rmsValue).toFixed(3)); color: "#e8ebee"; font.pixelSize: 13 }
        Text { text: qsTr("Tepe-Tepe: %1").arg((root.liveMode ? (root.liveMaxValue - root.liveMinValue) : Dispatcher.signalLoader.peakToPeakValue).toFixed(3)); color: "#e8ebee"; font.pixelSize: 13 }
        Item { Layout.fillWidth: true }
    }
    ...
    FileDialog {
        ...
        onAccepted: Dispatcher.signalLoader.loadFile(file)
    }

    // Static file plot - full rebuild each time a new file loads.
    Connections {
        target: Dispatcher.signalLoader
        function onDataChanged() {
            if (root.liveMode)
                return
            series.clear()
            const times = Dispatcher.signalLoader.timeValues
            const amps = Dispatcher.signalLoader.amplitudeValues
            ...
            axisY.min = Dispatcher.signalLoader.minValue
            axisY.max = Dispatcher.signalLoader.maxValue
        }
    }
